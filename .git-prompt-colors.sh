...
override_git_prompt_colors() {
  ...
  GIT_PROMPT_STAGED="${Red} staged "           # the number of staged files/directories
  GIT_PROMPT_CONFLICTS="${Red} conflicts "       # the number of files in conflict
  GIT_PROMPT_CHANGED="${Blue} changed "        # the number of changed files

  # GIT_PROMPT_REMOTE=" "                 # the remote branch name (if any) and the symbols for ahead and behind
  GIT_PROMPT_UNTRACKED="${Cyan} … "       # the number of untracked files/dirs
  GIT_PROMPT_STASHED="${BoldBlue} stashed "    # the number of stashed files/dir
  GIT_PROMPT_CLEAN="${BoldGreen} ok "      # a colored flag indicating a "clean" repo
  ...
  GIT_PROMPT_COMMAND_FAIL="${Red} failed -_LAST_COMMAND_STATE_ "    # indicator if the last command returned with an exit code of other than 0
